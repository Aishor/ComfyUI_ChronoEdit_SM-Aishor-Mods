...
class ChronoEditPipeline(DiffusionPipeline, WanLoraLoaderMixin):
    ...
    def prepare_latents(
        ...
        if image is not None:
            image = image.unsqueeze(2) # (B, C, T, H, W)
        ...
        if image_latent is not None:
            latent_condition = image_latent.to(device=device, dtype=dtype) #1, 16, 2, 60, 104
            print('latent_condition', latent_condition.shape)
        else:
        ...
        mask_lat_size = torch.ones(batch_size, 1, num_frames, latent_height, latent_width) # 1 ，1，5，60，104
